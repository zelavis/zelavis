...
import { fastifyIntegration } from "../dist/integrations/fastify.js";

test("fastifyIntegration mounts Zelavis as a Fastify plugin and preserves normalized request context", async () => {
  ...
  await app.register(fastifyIntegration(runtime));
...
test("fastifyIntegration preserves binary bodies, repeated headers, and HEAD fallback", async () => {
  const app = Fastify();
  const runtime = await zelavisServer({
    services: [
      defineServerService({
...
test("fastifyIntegration uses the configured error handler", async () => {
  const app = Fastify();
  const runtime = await zelavisServer({
    services: [
      defineServerService({
