...
import { elysiaIntegration } from "../dist/integrations/elysia.js";

test("elysiaIntegration mounts Zelavis as an Elysia plugin and preserves normalized request context", async () => {
...
test("elysiaIntegration preserves binary bodies, repeated headers, and HEAD fallback", async () => {
  const runtime = await zelavisServer({
    services: [
      defineServerService({
        name: "demo",
  ...
  const app = new Elysia().use(elysiaIntegration(runtime));
...
test("elysiaIntegration uses the configured error handler", async () => {
  const runtime = await zelavisServer({
    services: [
      defineServerService({
        name: "demo",
