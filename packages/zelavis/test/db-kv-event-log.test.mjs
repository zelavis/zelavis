// The event-log contract on the key-value path: source of truth, replication,
// cursor continuation and writer fencing all have to hold identically.
...
import { makeKvStore } from "../dist/db/engines/kv-node.js";
...
test("kv event log is the source of truth: lenses rebuild from it alone", async (t) => {
  const dir = tempDir(t);
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        const store = yield* makeKvStore("acme", dir);
        yield* ingest(store);
...
test("kv log replicates to a follower, and applying twice changes nothing", async (t) => {
  const dir = tempDir(t);
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        const leader = yield* makeKvStore("leader", dir);
        const follower = yield* makeKvStore("follower", dir);
        yield* ingest(leader);
...
test("kv cursors continue exactly and are rejected across partitions", async (t) => {
  const dir = tempDir(t);
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        const a = yield* makeKvStore("alpha", dir);
        const b = yield* makeKvStore("beta", dir);
        yield* ingest(a);
...
test("kv: a superseded writer is fenced", async (t) => {
  const dir = tempDir(t);
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        const stale = yield* makeKvStore("acme", dir);
        assert.equal(stale.generation, 1);

        // Opening the partition again claims the next generation.
        const current = yield* makeKvStore("acme", dir);
