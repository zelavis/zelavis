// The object-store contract over LMDB: a memory-mapped B+tree held to the same
// assertions as the LSM and SQL engines.
...
test("lmdb: one payload, many lenses, resolved through a shared seq space", async (t) => {
...
test("lmdb: retraction leaves no posting behind", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "zv-db-"));
  t.after(() => rmSync(dir, { recursive: true, force: true }));
