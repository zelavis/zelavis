...
test("createNodeHttpServer creates a native HTTP server and passes normalized request context", async () => {
...
test("createNodeHttpServer returns 404 for unmatched routes", async () => {
  const runtime = await createServiceRuntime({
    services: [],
  });
  const server = createNodeHttpServer(runtime);
...
test("createNodeHttpServer supports HEAD fallback, binary bodies, and repeated set-cookie headers", async () => {
  const runtime = await createServiceRuntime({
    services: [
      {
        name: "demo",
...
test("createNodeHttpServer supports trailing wildcard route params", async () => {
  const runtime = await createServiceRuntime({
    services: [
      {
        name: "demo",
...
test("createNodeHttpServer uses the configured error handler", async () => {
  const runtime = await createServiceRuntime({
    services: [
      {
        name: "demo",
