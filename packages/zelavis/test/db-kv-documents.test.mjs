// The document API over the key-value store: every layer above the store runs
// unchanged, which is the point of porting the store rather than the stack.
...
test("kv collections: create, list, reject reserved and malformed names", async (t) => {
...
test("kv documents: insert, findById, update merge/replace, delete", async (t) => {
  await withDocs(t, (docs) =>
    Effect.gen(function* () {
...
test("kv documents: identity, missing collections and optimistic concurrency", async (t) => {
  await withDocs(t, (docs) =>
    Effect.gen(function* () {
...
test("kv findMany: eq and in are index-served, comparisons are applied after", async (t) => {
  await withDocs(t, (docs) =>
    Effect.gen(function* () {
...
test("kv documents survive a lens rebuild from the event log", async (t) => {
