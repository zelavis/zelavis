// The object-store contract, run against the key-value path. Same assertions,
// different storage: a difference here is a difference in the store itself.
...
test("kv store: one payload, many lenses, resolved through a shared seq space", async (t) => {
...
test("kv store: retraction leaves no posting behind", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "zv-db-"));
  t.after(() => rmSync(dir, { recursive: true, force: true }));
