...
test("createServiceRuntime resolves promised services and returns service map plus routes", async () => {
...
test("createServiceRuntime denies protected routes without an authenticated principal", async () => {
  const runtime = await createServiceRuntime({
    services: [
      {
        name: "server",
...
test("createServiceRuntime allows project-scoped grants on matching project routes", async () => {
  const runtime = await createServiceRuntime({
    services: [
      {
        name: "projects",
...
test("createServiceRuntime parses multipart payloads and preserves repeated response headers", async () => {
  const runtime = await createServiceRuntime({
    services: [
      {
        name: "demo",
...
test("createServiceRuntime prefers an exact route over a wildcard sibling", async () => {
  const runtime = await createServiceRuntime({
    services: [
      {
        name: "demo",
