...
test("libsql adapter exposes a working documents API", async () => {
  ...
  const fetched = await database.forTenant("default").documents.findById({
    collection: "products",
    id: "prod_1",
  });
...
test("libsql adapter pushes filter and sort into SQL via json_extract", async () => {
  ...
  await database.forTenant("default").documents.createCollection({ name: "items" });
  ...
  const cheapAvailable = await database.forTenant("default").documents.findMany({
    collection: "items",
    where: [
  ...
  const descending = await database.forTenant("default").documents.findMany({
    collection: "items",
...
test("libsql adapter detects revision conflicts on document upsert", async () => {
  const database = await freshDatabase();
  await database.forTenant("default").documents.createCollection({ name: "notes" });
  await database.forTenant("default").documents.insert({
    collection: "notes",
    id: "n1",
    data: { body: "first" },
...
test("libsql adapter reports false when deleting unknown documents", async () => {
  const database = await freshDatabase();
  await database.forTenant("default").documents.createCollection({ name: "ghosts" });
  const deleted = await database.forTenant("default").documents.delete({
    collection: "ghosts",
    id: "missing",
  });
...
test("libsql adapter persists data through a transaction and rolls back on errors", async () => {
  const database = await freshDatabase();
  await database.forTenant("default").documents.createCollection({ name: "txns" });
  ...
  const after = await database.forTenant("default").documents.findById({
    collection: "txns",
    id: "row_1",
  });
