...
import type { DatabaseEvent, DatabaseEventCursor } from "../contracts/events.js";
import {
...
export class TimeSeriesService implements DatabaseTimeSeriesDefinitionsApi {
  ...
  private async derivePoints(
    ...
    const collection =
      definition.source?.collections?.length === 1
        ...
        : undefined;
    const sourceEvents = await readAllEvents(
      events,
      encodeDatabaseEventCursor(afterSequence),
      collection,
    );
    ...
    for (const event of sourceEvents) {
      lastSequence = decodeDatabaseEventCursor(event.cursor).position;
      ...
      for (const [pointIndex, point] of batch.entries()) {
        points.push({
          sourceSequence: decodeDatabaseEventCursor(event.cursor).position,
          pointIndex,
  ...
  private async syncStoredSeries(
    ...
    const batch = await this.derivePoints(definition, events, afterSequence);
  ...
  forTenant(tenantId: string, events: DatabaseEventsApi): DatabaseTenantTimeSeriesApi {
    return {
      get: (name) => this.getForTenant(name, tenantId, events),
    };
  }

  private getForTenant(
    name: string,
    tenantId: string,
    events: DatabaseEventsApi,
  ): DatabaseTimeSeriesHandle {
    ...
    return {
      range: async (input: DatabaseTimeSeriesRangeInput = {}) => {
        const version = await this.syncStoredSeries(definition, tenantId, events);
        ...
        const points = (await this.derivePoints(definition, events)).points.map(
          (entry) => entry.point,
        );
      ...
      aggregate: async (input: DatabaseTimeSeriesAggregateInput) => {
        ...
        const values = filterPoints(
          (await this.derivePoints(definition, events)).points.map(
            (entry) => entry.point,
          ),
          input,
