...
- `index.ts` creates a Fastify instance, constructs `new Zelavis({ adapter: nodeAdapter() })`, and registers `fastifyPlugin(zelavis)` (from `zelavis/fastify`) via `app.register(...)`
