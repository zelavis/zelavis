...
- This example does not need a Next.js adapter because App Router route handlers are already fetch-native: they receive a standard `Request` and return a standard `Response`, so `zv.fetch(request)` is already the cleanest integration path.
