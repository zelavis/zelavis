---
title: Service and Plugin Authoring
---
...
The repo includes `examples/plugin-basic` as a minimal uploadable plugin. Build its upload package with `pnpm --filter @zelavis/example-plugin-basic package`, then select `examples/plugin-basic/dist/example-basic.zip` in the Node example Marketplace flow. The plugin module defines its own `name`, `version`, menu, pages, and services, so the dashboard does not ask for a separate plugin name. The ZIP includes `zelavis.plugin.json`, whose `entry` field points at the ESM module the host adapter should import.
