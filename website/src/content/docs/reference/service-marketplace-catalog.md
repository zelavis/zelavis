...
For local testing, `examples/plugin-basic` builds a real service package at `examples/plugin-basic/dist/example-basic.zip`. Run `pnpm --filter @zelavis/example-plugin-basic package`, select that ZIP in the project Marketplace at `/zelavis/projects/:projectId/marketplace`, and the Node adapter will unpack it into `.zelavis/services`, read the package's `package.json` (`zelavis` field and `exports`), derive the service name from the module definition, then activate it without restarting the server.
